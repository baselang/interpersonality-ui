...
class CouplesIntroduction extends Component {
	constructor() {
		super();
		this.state = {
			language_id: ''
					...
					<div className="profile-content">
						<div className="contetnttt_moduleee profile-content__text pc--content">
							<div className="pc--heading">
								<h1><span>{String.fromCodePoint(url.COUPLES_GUIDE_LEFT_NAV_EMOJIS[0])}</span>{strings[url.COUPLES_GUIDE_LEFT_NAV[Object.keys(url.COUPLES_GUIDE_LEFT_NAV)[0]]]}</h1>
							</div>
							{renderHTML(strings.GUIDES_COUPLE_REPORT_INTRO_DESC)}
							<h2>{dynamicDivArray.length > 0 ? strings.GUIDES_COUPLE_REPORT_INTRO_DYNAMIC_CONTENT_HEADING : ''}</h2>
							{(renderHTML(dynamicDivArray.length > 0 ?  dynamicDivArray : ''))}
							<div className="profile-content__nav">
								<Link to="#" onClick={() => this.props.couplesActiveTab(Object.keys(url.COUPLES_GUIDE_LEFT_NAV)[1])}>
									<span className="profile-content__nav-icon">{String.fromCodePoint(url.COUPLES_GUIDE_LEFT_NAV_EMOJIS[1])}</span>
									<span>{strings.GUIDES_REPORT_NEXTSECTION}</span>
									<span className="profile-content__nav-title">{strings[url.COUPLES_GUIDE_LEFT_NAV[Object.keys(url.COUPLES_GUIDE_LEFT_NAV)[1]]]}</span>
								</Link>
							</div>
						</div>
					</div>
...
export default CouplesIntroduction;
