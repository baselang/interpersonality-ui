					...
					<div className="profile-content">
						<div className="contetnttt_moduleee profile-content__text pc--content">
							<div className="pc--heading">
								<h1><span>{String.fromCodePoint(url.COUPLES_GUIDE_LEFT_NAV_EMOJIS[1])}</span>{strings[url.COUPLES_GUIDE_LEFT_NAV[Object.keys(url.COUPLES_GUIDE_LEFT_NAV)[1]]]}</h1>
							</div>
							<div>
								{renderHTML(strings.GUIDES_COUPLE_REPORT_CONFLICT)}
							</div>
							{(renderHTML(dynamicDivArray.length > 0 ?  dynamicDivArray : ''))}
							<div className="profile-content__nav">
								<Link to="#" onClick={() => this.props.couplesActiveTab(Object.keys(url.COUPLES_GUIDE_LEFT_NAV)[2])}>
									<span className="profile-content__nav-icon">{String.fromCodePoint(url.COUPLES_GUIDE_LEFT_NAV_EMOJIS[2])}</span>
									<span>{strings.GUIDES_REPORT_NEXTSECTION}</span>
									<span className="profile-content__nav-title">{strings[url.COUPLES_GUIDE_LEFT_NAV[Object.keys(url.COUPLES_GUIDE_LEFT_NAV)[2]]]}</span>
								</Link>
							</div>
						</div>
					</div>
