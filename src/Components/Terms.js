...
class Terms extends Component {
    ...
    componentDidMount() {
        ...
        jQuery("body").addClass("template template-terms ")
        if(auth) {
    ...
    render() {
        ...
        let headContent = linkHeader.map((lin, index) => {
            return (
                <link key={index} rel={lin.rel} hreflang={lin.hreflang} href={url.ORIGIN_URL+lin.href+"terms"}/>
            )
        })
        ...
        if(test_status == "not_completed") {
            ...
            return (
                <div>
                    <Helmet>
                        {headContent}
                        <title>{strings.TERMS_META_TITLE}</title>
                        <meta name="description" content={strings.TERMS_META_DESCRIPTION} />
                    </Helmet>
                    ...
                    <main id="generic-content">
                        <div className="container">
                            <div className="entry-title text-center">
                                ...
                                <h1>{ strings.TERMS_TITLE }</h1>
                            </div>
                            <div className="entry-content">
                            {renderHTML(strings.TERMS_DESC)}
