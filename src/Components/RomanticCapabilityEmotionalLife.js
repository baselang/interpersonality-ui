...
class RomanticCapabilityEmotionalLife extends Component {
    constructor() {
		super();
		this.state = {
			language_id: ''
    ...
    render() {
		...
		let dynamicDivArray = [];
		let report_length = this.props.guides_report[3] != undefined ?   this.props.guides_report[3].length : ''

		if(report_length != undefined) {
			this.props.guides_report[3] && this.props.guides_report[3].map((data, index)=>{
					...
					<div className="profile-content">
						<div className="contetnttt_moduleee profile-content__text pc--content">
							<div className="pc--heading">
								<h1><span>{String.fromCodePoint(url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV_EMOJIS[3])}</span>{strings[url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV[Object.keys(url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV)[3]]] }</h1>
							</div>
							{(renderHTML(dynamicDivArray.length > 0 ?  dynamicDivArray : ''))}
							<div className="profile-content__nav">
								<Link to="#" onClick={() => this.props.romaticCapabilityActiveTab(Object.keys(url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV)[4])}>
									<span className="profile-content__nav-icon">{String.fromCodePoint(url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV_EMOJIS[4])}</span>
									<span>{strings.GUIDES_REPORT_NEXTSECTION}</span>
									<span className="profile-content__nav-title">{strings[url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV[Object.keys(url.ROMANTIC_COMPATABILITY_GUIDE_LEFT_NAV)[4]]]}</span>
								</Link>
							</div>
						</div>
					</div>
...
export default RomanticCapabilityEmotionalLife;
