...
class OurTeam extends Component {
    ...
    componentDidMount() {
        ...
        jQuery("body").addClass("template template-our-team")
    ...
    render() {
        ...
        let headContent = linkHeader.map((lin, index) => {
            return (
                <link key={index} rel={lin.rel} hreflang={lin.hreflang} href={url.ORIGIN_URL+lin.href+"our-team"}/>
            )
        })
        ...
        if(test_status == "not_completed") {
            ...
            return (
                <div>
                    <Helmet>
                        {headContent}
                        <title>{strings.OUR_TEAM_META_TITLE}</title>
                        <meta name="description" content={strings.OUR_TEAM_META_DESCRIPTION} />
                    </Helmet>
                    ...
                    <main id="generic-content">
                        <div className="container">
                            <div className="entry-title text-center">
                                ...
                                <h1>{strings.OURTEAM_TITLE}</h1>
                            </div>
                            <div className="entry-content">
                                {renderHTML(strings.OURTEAM_DESC)}
                            </div>
                        </div>
                    </main>
                    {!this.state.auth &&
