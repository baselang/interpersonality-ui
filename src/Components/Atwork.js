    ...
    render() {
		...
		let ctaContent = '';
			...
			ctaContent = share_module_report_content.find((itmInner) => {
				return (itmInner.Section == 5 ) ? itmInner : "";
			});
		}

		let Share_CTA = (ctaContent != '' && ctaContent != undefined ) ? <ProfileShareCTA Content = {ctaContent.Content}/>:"";
		let dynamicDivArray = [];
		let report_length = ( this.props.user_report_data != undefined && this.props.user_report_data[5] != undefined )?   this.props.user_report_data[5].length : 0;
		
		if(report_length > 0 ) {
			this.props.user_report_data[5].map((story, index)=>{
					...
					<div className="profile-content">
						<div className="contetnttt_moduleee profile-content__text pc--content">
							<div className="pc--heading">
								<h1><span>💼</span>{strings[url.PROFILE_LEFT_NAV[Object.keys(url.PROFILE_LEFT_NAV)[5]]]}</h1>
							</div>
							{[(dynamicDivArray)]}
							<div className="profile-content__nav">
								<Link to="#" onClick={() => this.props.activateTab(Object.keys(url.PROFILE_LEFT_NAV)[6])}>
									<span className="profile-content__nav-icon">👥</span>
									<span>{strings.NEXT_SECTION}</span>
									<span className="profile-content__nav-title">{strings[url.PROFILE_LEFT_NAV[Object.keys(url.PROFILE_LEFT_NAV)[6]]]}</span>
								</Link>
							</div>
						</div>
					</div>
